...
contract DeliHookConstantProduct is Ownable2Step, MultiPoolCustomCurve {
        ...
        returns (uint256)
    {
        ...
        if (params.amountSpecified < 0) {
            ...
            if (feeFromOutput) {
                // Compute fee = ceil(netOut * f / (1 - f)), then grossOut = netOut + fee
                uint256 feeOut =
                    FullMath.mulDivRoundingUp(amountSpecified, uint256(key.fee), 1_000_000 - uint256(key.fee));
                uint256 grossOut = amountSpecified + feeOut;
    ...
    function _updateReservesAfterSwap(PoolId poolId, V2Pool storage pool, PoolKey memory key) private {
        ...
        if (exactInput) {
            ...
            uint256 amountIn;
            if (_feeFromOutput) {
                // Compute input against gross output
                // fee = ceil(netOut * f / (1 - f)), grossOut = netOut + fee
                uint256 feeOut =
                    FullMath.mulDivRoundingUp(specifiedAmount, uint256(effectiveFee), 1_000_000 - uint256(effectiveFee));
                uint256 grossOut = specifiedAmount + feeOut;
    ...
    function _calculateImplicitFee(PoolKey calldata key, SwapParams calldata params) private {
        ...
        if (exactInput) {
            ...
            if (_feeFromOutput) {
                // EXACT OUTPUT + FEE FROM OUTPUT: fee = ceil(netOut * f / (1 - f))
                feeAmount = FullMath.mulDivRoundingUp(amountSpecified, uint256(key.fee), 1_000_000 - uint256(key.fee));
            } else {
        ...
        returns (uint256 amountIn)
    {
        ...
        (bool feeFromOutput,) = _isFeeFromOutput(key, zeroForOne);
        if (feeFromOutput) {
            // feeOut = ceil(netOut * f / (1 - f)); grossOut = netOut + feeOut;
            uint256 feeOut = FullMath.mulDivRoundingUp(amountOut, uint256(key.fee), 1_000_000 - uint256(key.fee));
            uint256 grossOut = amountOut + feeOut;
            uint256 rOut = zeroForOne ? uint256(pool.reserve1) : uint256(pool.reserve0);
        ...
        returns (uint256[] memory amounts)
    {
        ...
        for (uint256 i = n; i > 0;) {
            ...
            if (feeFromOutput) {
                // feeOut = ceil(netOut * f / (1 - f)); grossOut = netOut + feeOut;
                uint256 feeOut =
                    FullMath.mulDivRoundingUp(amounts[i + 1], uint256(route[i].fee), 1_000_000 - uint256(route[i].fee));
                uint256 grossOut = amounts[i + 1] + feeOut;
